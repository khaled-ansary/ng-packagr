...
export async function transformSources(ngPkg: NgPackageData): Promise<void> {
  ...
  const fesm15File = path.resolve(ngPkg.buildDirectory, `${ngPkg.fullPackageName}.js`);
  await rollup({
  ...
  const fesm5File = path.resolve(ngPkg.buildDirectory, `${ngPkg.fullPackageName}.es5.js`);
  ...
  await writePackage(ngPkg, {
    ...
    module: ensureUnixPath(path.join(rootPathFromSelf, `${ngPkg.fullPackageName}.es5.js`)),
    es2015: ensureUnixPath(path.join(rootPathFromSelf, `${ngPkg.fullPackageName}.js`)),
